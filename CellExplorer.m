...
%                 subfig_ax(1).XLabel.String = UI.plot.xTitle(1:end-4); 
    ...
    if strcmp(UI.panel.subfig_ax(2).Visible,'on')
        ...
        elseif ~strcmp(UI.preferences.groundTruthData, 'None') && ~strcmp(UI.preferences.referenceData, 'None')
            ...
            %fig2_axislimit_y doesn't exist, couldnt figure out where to
            %set in my defaults so replaced with the x axis limit value
            %xlim(fig2_axislimit_x), ylim(fig2_axislimit_y)
             xlim(fig2_axislimit_x), ylim(fig2_axislimit_x)
        end
        xlim21 = xlim;
        ylim21 = ylim;
...
%                 subfig_ax(1).YTick = [0:0.1:1];
    ...
    function initializeSession
        ...
        %using this dataset+code yields some units that are empty, this code
        %replacement skips those
        ...
        cell_metrics.waveforms.time_zscored = [max(cellfun(@min, cell_metrics.waveforms.time(find(~cellfun(@isempty,cell_metrics.waveforms.time))))):min([step_size{:}]):min(cellfun(@max, cell_metrics.waveforms.time(find(~cellfun(@isempty,cell_metrics.waveforms.time)))))];
        ...
        if ~cell_metrics.general.initialized | ~isfield(cell_metrics.waveforms,{'filt_zscored','filt_absolute'})
            ...
            for i = 1:length(cell_metrics.waveforms.filt)
                 %using this dataset+code yields some units that are empty,
                 %this try+catch skips those
                try
