...
if any(contains(parameters.metrics,{'waveform_metrics','all'})) && ~any(contains(parameters.excludeMetrics,{'waveform_metrics'}))
    ...
    if ~all(isfield(cell_metrics,{'trilat_x','trilat_y','peakVoltage_expFit'})) || parameters.forceReload == true
        ...
        if parameters.debugMode
            ...
            title(handle_subfig1,'Spike amplitude (all)'), xlabel(handle_subfig1,'Distance (�m)'), ylabel(handle_subfig1,'�V')
            handle_subfig2 = subplot(2,2,3); hold on
            ylabel(handle_subfig2,'Length constant (�m)'), xlabel(handle_subfig2,'Peak voltage (�V)')
        ...
        title('Spike amplitude (all)'), xlabel('Distance (�m)'), ylabel('�V')
        subplot(2,2,2), hold off,
        histogram(cell_metrics.peakVoltage_expFit,20), xlabel('Length constant (�m)')
        subplot(2,2,3), hold on
        plot(cell_metrics.peakVoltage,cell_metrics.peakVoltage_expFit,'ok')
        ylabel('Length constant (�m)'), xlabel('Peak voltage (�V)')
        subplot(2,2,4), hold on
        plot(cell_metrics.general.chanCoords.x,cell_metrics.general.chanCoords.y,'.k'), hold on
        plot(cell_metrics.trilat_x,cell_metrics.trilat_y,'ob'), xlabel('x position (�m)'), ylabel('y position (�m)')
    end
    ...
    if exist(ccf_file,'file') %&& (~all(isfield(cell_metrics,{'ccf_x','ccf_y','ccf_z'})) || parameters.forceReload == true)
        ...
        xlabel('x ( Anterior-Posterior; �m)'), zlabel('y (Superior-Inferior; �m)'), ylabel('z (Left-Right; �m)'), axis equal, set(gca, 'ZDir','reverse')
