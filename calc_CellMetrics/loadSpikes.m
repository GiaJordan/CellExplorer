...
if parameters.forceReload
    ...
    switch lower(format)
        ...
        case {'mclust'} % MClust developed by David Redish
            ...
            UID = 0;
            %had to change to accomodate file name convention of Schimanski
            %dataset
            fileList = dir(fullfile(clusteringpath_full,'TT*c.mat'));
            fileList = {fileList.name};
            ...
            for iTetrode = 1:numel(fileList)
                ...
                %had to change to accomodate file name convention of Schimanski
            %dataset
                if exist([fileList{iTetrode}(1:end-4),'.clusters'],'file')
                    clusterData = load([fileList{iTetrode}(1:end-4),'.clusters'],'-mat');
                    %timeStampData = load(fullfile(clusteringpath_full,[fileList{iTetrode}(1:end-4),'_1.t']),'-mat');
                    
                    %                     f=fopen(fullfile(clusteringpath_full,[fileList{iTetrode}(1:end-4),'_1.t']));
                    %
                    %                     %Check type for importing
                    %                     timeStampData=fread(f,'uint16');
                    %                     fclose(f)
                    
                    %energy data is in .fd, amp is not present
                    energyData = load([fileList{iTetrode}(1:end-4),'_energy.fd'],'-mat');
                    ...
                    for i = 1:numel(clusterData.MClust_Clusters)
                        ...
                        f=fopen([fileList{iTetrode}(1:end-4),'_',num2str(i), '.t']);
                        
                        %Check type for importing
                        spikes.times{UID}=sort(fread(f,'uint16')');
                        fclose(f);
                        ...
                        %%%Dont have enought info from tetrode loading.
                        %%%dont think aggregating cluster time stamp info
                        %%%will work for real
                        %spikes.filtWaveform_all{UID} = permute(mean(tetrodeData.WaveForms(clusterData.MClust_Clusters{i}.myPoints,:,:)),[3,2,1])';
                        
                        tetName=fileList(iTetrode);
                        tetName=tetName{1};
                        tetName=tetName(1:5);
                        clustPath=join([tetName,'_ClusterSummary_',num2str(i),'.mat']);
                        cSumm=load(clustPath);
                        cSumm=cSumm.CI;
                        spikes.filtWaveform_all{UID} = permute(cSumm.MeanWaveform,[3,2,1])';
                        %spikes.filtWaveform_all{UID} = cSumm.MeanWaveform;
                        
                        
                        %changed from braces to parenths

                        spikes.channels_all{UID} = session.extracellular.electrodeGroups.channels{1}(iTetrode);
                        ...
                        %have to load peaks from appropriate .fd file
                        f=fopen([fileList{iTetrode}(1:end-4),'_peak.fd']);
                        
                        %Check type for importing
%                         val=fread(f,'uint16');
%                         if isempty(val)
%                             spikes.peakVoltage(UID)=0;
%                         else
%                             spikes.peakVoltage(UID) = max(val)
%                         end
%                         fclose(f)
%                         clear val
                        ...
                        %same thing with energy .fd file
                        f=fopen([fileList{iTetrode}(1:end-4),'_energy.fd']);
                        
                        %Check type for importing
                        spikes.energy{UID}=fread(f,'uint16');
                        fclose(f)
